...
export default class ImageTiler {
    palettes: number[] = []
    unik_tiles: TileHash[] = []
    tiles: TileHash[] = []
    imageUrl?: string
    ...
    getStats() {
        return {
            imageH: this.imageH,
            imageW: this.imageW,
            paletteSize: this.palettes.length,
            tilesCount: this.unik_tiles.length
        }
    }
    ...
    async build() {
        ...
        // flip palette order for fast lookup
        const palette_flip = Array.from(this.palettes.keys()).reduce((obj: Record<number, number>, v) => {
            obj[this.palettes[v]] = v;
            return obj
        }, {})

        // output palette
        this.palettes.forEach(c => palData.addUint16(c))
        ...
        // limite tiles to 400
        const vdptiles = this.unik_tiles.slice(0, 0x400)


        // tiles to palette
        vdptiles.forEach(({ tile }) => {
            const uint4b: number[] = []
        ...
        header.addUint16(this.unik_tiles.length)
...
export async function old_ImageTiler(i: any) {
