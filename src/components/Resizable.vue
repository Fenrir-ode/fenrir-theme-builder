<template>
  <div ref="parent" class="resizable-component" :style="style">
    <slot></slot>
    <div v-for="el in active" v-show="!maximize" :key="el" :class="'resizable-' + el"></div>
  </div>
</template>
...
export default {
  ...
  props: {
    ...
    active: {
      ...
      validator: (val) =>
        ['r', 'rb', 'b', 'lb', 'l', 'lt', 't', 'rt'].filter((value) => val.indexOf(value) !== -1)
          .length === val.length,
      type: Array
    },
    fitParent: {
    ...
    disableAttributes: {
      default: () => [],
      validator: (val) =>
        ['l', 't', 'w', 'h'].filter((value) => val.indexOf(value) !== -1).length === val.length,
      type: Array
    }
  },
  emits: [
